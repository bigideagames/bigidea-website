...
// Soccer uses the same UI flow screenshots as hockey — identical delete flow
const STEPS = [
  {
    num: 1,
...
export default function DeleteAccountSoccerPage() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
          ...
          {STEPS.map((step) => (
            <div key={step.num} className="flex flex-col gap-4">
              <div className="flex items-start gap-4">
                <span className="flex-shrink-0 w-10 h-10 rounded-full bg-[#22C55E] text-white font-bold flex items-center justify-center text-lg">
                  {step.num}
                </span>
